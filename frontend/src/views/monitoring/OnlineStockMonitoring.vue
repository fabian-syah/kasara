...
import { Loader2, Globe, MapPin, Box, Smartphone, Activity, ChevronDown } from "lucide-vue-next";
...
const fetchData = async () => {
    ...
    try {
        const response = await api.get('/monitoring/online-shop');
    ...
    <div class="space-y-6 animate-in pb-20">
        ...
        <div>
            <h1 class="text-2xl font-bold text-text-primary tracking-tight flex items-center gap-2">
                <Globe :size="28" class="text-cyan-500" /> Monitoring Stok Online
            </h1>
            <p class="text-text-secondary mt-1 max-w-2xl">
                Pantau ketersediaan stok barang di toko online yang Anda kelola.
            </p>
        </div>

        <!-- Loading State -->
        <div v-if="isLoading" class="flex flex-col items-center justify-center py-20">
            <Loader2 class="w-10 h-10 animate-spin text-cyan-500 mb-4" />
            <p class="text-text-secondary animate-pulse font-medium">Memuat data stok online...</p>
        </div>
        ...
        <template v-else>
            ...
            <div class="grid grid-cols-2 gap-4 mb-6">
                <div
                    class="bg-gradient-to-br from-cyan-500/10 to-cyan-600/5 dark:from-cyan-500/20 dark:to-cyan-900/10 border border-cyan-500/20 dark:border-cyan-500/30 p-5 rounded-2xl flex items-center gap-4 shadow-sm">
                    <div
                        class="w-12 h-12 rounded-xl bg-cyan-500/20 text-cyan-600 dark:text-cyan-400 flex items-center justify-center">
                        <MapPin :size="24" />
                    </div>
                    <div>
                        <p class="text-sm font-medium text-text-secondary">Total Toko</p>
                        <p class="text-2xl font-black text-text-primary">{{ totalLocations }} <span
                                class="text-sm font-semibold text-text-secondary">Toko</span></p>
                    </div>
                </div>
            ...
            <div v-if="!monitoringData.stock || !monitoringData.stock.length"
                ...
                <p class="text-text-secondary max-w-sm">
                    Belum ada produk yang tersedia di toko online yang Anda kelola.
                </p>
            </div>
            ...
            <div v-else class="space-y-4">
                <div v-for="(location, idx) in monitoringData.stock" :key="idx"
                    class="relative overflow-hidden bg-white dark:bg-surface-800/60 backdrop-blur-3xl border border-surface-200 dark:border-surface-700/50 shadow-sm dark:shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-2xl transition-all duration-300 hover:shadow-md dark:hover:border-cyan-500/30">
                    ...
                    <div @click="toggleLocation(idx)"
                        ...
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <span
                                    class="text-xs font-bold px-3 py-1 rounded-full bg-cyan-100 text-cyan-700 dark:bg-cyan-900/40 dark:text-cyan-400">
                    ...
                    <div v-show="expandedLocations[idx]" class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                            <div v-for="(product, pIdx) in location.products" :key="pIdx"
                                class="group flex flex-col p-4 rounded-xl bg-surface-50 dark:bg-surface-900 border border-transparent dark:border-surface-700/30 transition-all duration-300 hover:bg-surface-100 hover:border-surface-200 dark:hover:bg-surface-800 dark:hover:border-cyan-500/20 shadow-sm">

                                <div class="flex items-center justify-between w-full">
                                    <div class="flex items-center flex-1 pr-4 gap-3">
                                        <div
                                            class="w-10 h-10 rounded-lg bg-surface-200 dark:bg-surface-800 flex items-center justify-center text-text-secondary group-hover:text-cyan-500 group-hover:bg-cyan-500/10 transition-colors">
                                            <Smartphone v-if="product.type === 'hp'" :size="18" />
                                            <Box v-else :size="18" />
                                        </div>
                                        <div class="flex flex-col">
                                            <h4
                                                class="font-semibold text-text-primary text-sm tracking-tight group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors leading-snug">
                                    ...
                                    <div class="flex flex-col items-end justify-center min-w-[60px]">
                                        <span class="text-2xl font-black text-cyan-600 dark:text-cyan-400 leading-none">
                                            {{ product.qty }}
                                        </span>
                                        <span
                                ...
                                <div v-if="product.items && product.items.length > 0"
                                    ...
                                    <div class="flex flex-col gap-2 max-h-40 overflow-y-auto pr-1 custom-scrollbar">
                                        <div v-for="item in product.items" :key="item.id"
                                            class="flex items-center justify-between py-2 px-3 rounded-lg bg-white dark:bg-surface-800/80 border border-surface-200 dark:border-surface-700/60 shadow-sm hover:border-cyan-300 dark:hover:border-cyan-700 transition-colors">
