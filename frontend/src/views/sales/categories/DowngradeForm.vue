...
function addItem() {
    additionalItems.value.push({
        ...
        quantity: 1,
        buy_price: 0,
        distributor_id: downgradeForm.value.distributor_id,
    });
}
...
async function handlePhotoChange(type, event) {
    ...
    if (file) {
        try {
            ...
            // 2. Set File Object
            downgradePhotos.value[type] = compressedFile;
...
async function submitDowngrade(pin = null) {
    if (
        isImeiDowngrade.value && 
        selectedOutgoingDowngrade.value?.imei && 
    ...
    if (additionalItems.value.length > 0) {
        ...
        // Main item
        allItems.push({
            brand_id: downgradeForm.value.incoming_brand_id,
            product_type_id: downgradeForm.value.incoming_product_type_id,
            imeis: isImeiDowngrade.value ? [downgradeForm.value.incoming_imei].filter(i => i) : [],
            quantity: isImeiDowngrade.value ? 1 : (downgradeForm.value.incoming_quantity || 1),
            storage: downgradeForm.value.incoming_storage,
            condition: downgradeForm.value.incoming_condition,
            buy_price: downgradeForm.value.incoming_cost_price,
            distributor_id: downgradeForm.value.distributor_id,
        });
        ...
        for (const item of additionalItems.value) {
            ...
            allItems.push({
                ...
                distributor_id: item.distributor_id || downgradeForm.value.distributor_id,
            });
        }
    ...
    try {
        ...
        let msg = "Gagal memproses downgrade";
        if (error.response) {
...
<template>
    <div class="flex-1 overflow-y-auto custom-scrollbar bg-white dark:bg-surface-800 rounded-[1.5rem] sm:rounded-[2rem] border border-surface-200 dark:border-surface-700 p-4 pb-24 sm:p-8 sm:pb-8 shadow-xl">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                        <div>
                            ...
                            <div @click="$refs.unitDGInput.click()" class="relative border-2 border-dashed border-surface-300 dark:border-surface-600 rounded-xl aspect-square flex flex-col items-center justify-center cursor-pointer hover:bg-surface-50 dark:hover:bg-surface-800 transition-all overflow-hidden group">
                                ...
                                <template v-else-if="downgradePhotos.unitPreview">
                        ...
                        <div>
                            ...
                            <div @click="$refs.customerDGInput.click()" class="relative border-2 border-dashed border-surface-300 dark:border-surface-600 rounded-xl aspect-square flex flex-col items-center justify-center cursor-pointer hover:bg-surface-50 dark:hover:bg-surface-800 transition-all overflow-hidden group">
                                ...
                                <template v-else-if="downgradePhotos.customerPreview">
