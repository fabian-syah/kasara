...
const filteredExchangeTypes = computed(() => {
    ...
    return productTypes.value.filter(t => t.brand_id === unitExchangeForm.value.incoming_brand_id);
});

const selectedExchangeType = computed(() => {
    if (!unitExchangeForm.value.incoming_product_type_id) return null;
    return productTypes.value.find(t => t.id === unitExchangeForm.value.incoming_product_type_id);
});
...
function nextStep() {
    ...
    if (currentStep.value === 3) {
        if (transactionCategory.value === 'tukar_unit') {
            if (!unitExchangeForm.value.outgoing_product_detail_id) {
                alert("Silakan Pilih 1 produk untuk ditukar.");
                return;
            }
    ...
    <div class="max-w-[1600px] mx-auto px-2 sm:px-4 py-4 sm:py-8 min-h-[calc(100vh-8rem)]">
        ...
        <div class="h-full flex flex-col transition-all duration-300">
            ...
            <div v-if="currentStep === 3" class="flex-1 flex flex-col lg:flex-row gap-8 min-h-0 animate-fade-in">
                ...
                <!-- TUKAR TAMBAH FORM (NEW REFINED LAYOUT) -->
