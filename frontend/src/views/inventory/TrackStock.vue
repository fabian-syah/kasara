                ...
                <div class="relative space-y-12 pb-8">
                    ...
                    <template v-for="result in results">
                        ...
                        <div v-if="result.type === 'stock_in'" :key="'in-' + result.id"
                            ...
                            :class="result.is_retur_rejection ? 'border-l-red-500 bg-red-500/5' : (result.is_return_transfer ? 'border-l-amber-500 bg-amber-500/5' : (result.is_arrival ? 'border-l-indigo-500 bg-indigo-500/5' : 'border-l-green-500'))">
                            <!-- Header -->
                            <div class="flex flex-col sm:flex-row items-start justify-between gap-4 mb-6">
                                <div class="flex items-center gap-3">
                                    <div class="w-12 h-12 rounded-xl flex items-center justify-center shrink-0"
                                        :class="result.is_retur_rejection ? 'bg-red-500/20 text-red-500' : (result.is_return_transfer ? 'bg-amber-500/20 text-amber-500' : (result.is_arrival ? 'bg-indigo-500/20 text-indigo-500' : 'bg-green-500/20 text-green-500'))">
                                        <ArrowUpRight v-if="!result.is_arrival && !result.is_return_transfer" :size="24" />
                                        <MapPin v-else :size="24" />
                                    </div>
                                    <div class="min-w-0">
                                        <div class="flex flex-wrap items-center gap-2 mb-1">
                                            <span v-if="result.is_retur_rejection"
                                                class="text-red-400 text-[10px] font-bold bg-red-500/10 border border-red-500/20 px-2 py-0.5 rounded uppercase tracking-wider">RETUR DITOLAK / KEMBALI KE ASAL</span>
                                            <span v-else-if="result.is_return_transfer"
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                                ...
                                <div>
                                    <p class="text-text-secondary text-xs flex items-center gap-1">
                                        <User :size="12" /> {{ result.is_retur_rejection ? 'Ditolak oleh' : (result.is_return_transfer ? 'Diterima Balik oleh' : (result.is_arrival ? 'Diterima oleh' : 'Diinput oleh')) }}
                                    </p>
