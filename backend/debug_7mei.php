...
$branch = Branch::where('name', 'ilike', '%pekanbaru%')->first();
if (!$branch) {
...
// Query similar to AuditController's rawStatsQuery
$rawStatsQuery = DB::table('stock_outs')
    ...
    ->where('stock_outs.branch_id', $branchId);
...
$rawStats = $rawStatsQuery->whereIn('stock_outs.category', $salesCategories)
    ->select('id', 'payment_method_id', 'category', 'selling_price', 'split_payments', 'notes', 'sales_account')
    ->get();
...
foreach ($rawStats as $ps) {
    ...
    echo "ID: {$ps->id}, OrigCat: {$ps->category}, ResolvedCat: {$cat}, Price: {$price}, SplitAmt: {$amt}, Notes: {$ps->notes}\n";
}

echo "\n--- TOTALS ---\n";
echo "Base Sales Only: {$baseSalesOnly}\n";
echo "Trade Selisih: {$tradeSelisih}\n";
echo "Deductions: {$deductions}\n";
echo "Total Omset (Base + Trade): " . ($baseSalesOnly + $tradeSelisih) . "\n";
echo "Omset Bersih (Base - Deductions): " . ($baseSalesOnly - $deductions) . "\n";
