...
class AuditController extends Controller
{
    ...
    public function sales(Request $request)
    {
        try {
            ...
            $normalizedSalesCategories = array_unique(array_map(function ($c) {
                return strtolower(str_replace(' ', '_', $c)); }, $salesCategories));
            ...
            [$paginatedSales, $brandSalesRaw, $csSalesRaw, $dailyHistoryRaw, $typeStatsRaw, $conditionStatsRaw, $distributorStatsRaw, $soldProducts, $soldDistributors, $reportSummary] = Octane::concurrently([
                ...
                function () use ($successCategories, $startDate, $endDate, $branchIds, $onlineShopIds, $warehouseIds, $distributorIds, $requestedBranchId, $requestedOnlineShopId, $requestedWarehouseId, $requestedDistributorId, $isAnalist) {
                    ...
                    $applyQueryFilters = function ($q) use ($branchIds, $onlineShopIds, $warehouseIds, $distributorIds, $requestedBranchId, $requestedOnlineShopId, $requestedWarehouseId, $requestedDistributorId) {
                        if ($requestedBranchId) {
                            $q->where(function ($sq) use ($requestedBranchId) {
                                $sq->where('stock_outs.branch_id', $requestedBranchId)
                                    ->orWhere('users.branch_id', $requestedBranchId);
                    ...
                    $hp = DB::table('stock_out_items')
                        ->join('stock_outs', 'stock_out_items.stock_out_id', '=', 'stock_outs.id')
                        ->join('product_details', 'stock_out_items.product_detail_id', '=', 'product_details.id')
                        ->join('products', 'product_details.product_id', '=', 'products.id')
                        ->join('users', 'stock_outs.user_id', '=', 'users.id')
                        ->whereIn('stock_outs.category', $successCategories)
                        ->whereNull('stock_outs.deleted_at')
                        ->where(function ($q) use ($startDate, $endDate, $startTS, $endTS) {
                            $q->whereBetween('stock_outs.reporting_date', [$startDate, $endDate])
                                /* ->orWhereBetween('stock_outs.created_at', [$startTS, $endTS]) */ ;
                    ...
                    $nhp = DB::table('stock_out_non_hp_items')
                ...
                function () use ($salesCategories, $startDate, $endDate, $stockStartDate, $stockEndDate, $branchIds, $onlineShopIds, $warehouseIds, $distributorIds, $requestedBranchId, $requestedOnlineShopId, $requestedWarehouseId, $requestedDistributorId, $requestedLocationType, $paymentMethods, $distributors, $isUnrestricted, $isAnalist, $isSuperAdmin, $currentRoles, $isInventoryRole, $userId, $userName) {
                    try {
                        ...
                        $applyLocalScope = function ($query) use ($startDate, $endDate, $requestedBranchId, $requestedOnlineShopId, $requestedWarehouseId, $requestedDistributorId, $requestedLocationType, $branchIds, $onlineShopIds, $warehouseIds, $distributorIds, $isGlobalUnrestricted, $isAnalist, $isSuperAdmin, $isInventoryRole, $userId, $userName) {
                            ...
                            $query->where(function ($q) use ($requestedBranchId, $requestedOnlineShopId, $requestedWarehouseId, $requestedDistributorId, $requestedLocationType, $branchIds, $onlineShopIds, $warehouseIds, $distributorIds, $isGlobalUnrestricted, $isAnalist, $isSuperAdmin, $isInventoryRole) {
                                ...
                                if ($requestedBranchId) {
                                    $q->where(function ($sq) use ($requestedBranchId) {
                                        $sq->where('stock_outs.branch_id', $requestedBranchId)
                                            ->orWhereExists(fn($sub) => $sub->select(DB::raw(1))->from('users')->whereRaw('users.id = stock_outs.user_id')->where('users.branch_id', $requestedBranchId));
                                    });
                                } elseif ($requestedOnlineShopId) {
                                    $q->where(function ($sq) use ($requestedOnlineShopId) {
                                        $sq->where('stock_outs.online_shop_id', $requestedOnlineShopId)
                                            ->orWhereExists(fn($sub) => $sub->select(DB::raw(1))->from('users')->whereRaw('users.id = stock_outs.user_id')->where('users.online_shop_id', $requestedOnlineShopId));
                                    });
                                } elseif ($requestedWarehouseId) {
                                    $q->where(function ($sq) use ($requestedWarehouseId) {
                                        $sq->where('stock_outs.warehouse_id', $requestedWarehouseId)
                                            ->orWhereExists(fn($sub) => $sub->select(DB::raw(1))->from('users')->whereRaw('users.id = stock_outs.user_id')->where('users.warehouse_id', $requestedWarehouseId));
                                    });
                                } elseif ($requestedDistributorId) {
                                ...
                                } elseif ($isGlobalUnrestricted && !$isAnalist && !$isSuperAdmin) {
                        ...
                        $applyStockScope = function ($q) use ($requestedBranchId, $requestedOnlineShopId, $requestedWarehouseId, $requestedDistributorId, $branchIds, $onlineShopIds, $isGlobalUnrestricted, $isAnalist, $isSuperAdmin) {
                        ...
                        $applyInScope = function ($q) use ($startDate, $endDate, $requestedBranchId, $requestedOnlineShopId, $requestedWarehouseId, $requestedDistributorId, $branchIds, $onlineShopIds, $isGlobalUnrestricted, $isAnalist, $isSuperAdmin) {
                            $startTS = $startDate . ' 05:00:00';
                            $endTS = date('Y-m-d', strtotime($endDate . ' +1 day')) . ' 04:59:59';
                        ...
                        $omsetCategories = ['shopee', 'orderan_online', 'penjualan_offline', 'penjualan_store', 'pos', 'sale', 'SALE', 'POS', 'Sale', 'Pos', 'PENJUALAN_STORE', 'Penjualan_Store', 'tukar_unit', 'tukar_tambah', 'downgrade', 'bundling', 'cancel_penjualan', 'pelunasan_dp'];
                        ...
                        $resolveActualCategory = function ($category, $notes, $salesAccount) {
                            ...
                            if (in_array($category, ['shopee', 'orderan_online', 'penjualan_offline', 'penjualan_store', 'pos', 'sale', 'bundling', 'tukar_tambah', 'pelunasan_dp'])) {
                                return $category;
                            }
                        ...
                        $getCategoryByItem = function ($did, $isHp = false, $name = '') {
                            ...
                            if (str_contains($nameLower, 'icloud'))
                                return 'icloud';
                            if (str_contains($nameLower, 'jasa') || str_contains($nameLower, 'service') || str_contains($nameLower, 'servis'))
                                return 'jasa';
                            if (str_contains($nameLower, 'case') || str_contains($nameLower, 'charger') || str_contains($nameLower, 'kabel') || str_contains($nameLower, 'adaptor') || str_contains($nameLower, 'tempered'))
                                return 'accessories';
            ...
            $dailySales = collect($paginatedSales->items())->map(function ($trx) use ($branches, $onlineShops, $warehouses, $questions, $paymentMethods, $distributors, $ttData, $dgData, $ueData, $rfData) {
                ...
                $exchangeType = '';
                if ($catLower === 'tukar_tambah') {
                    $exchangeInfo = $ttData->get($receiptId);
                    $exchangeType = 'TUKAR TAMBAH';
                } elseif ($catLower === 'downgrade') {
                    $exchangeInfo = $dgData->get($receiptId);
                    $exchangeType = 'DOWNGRADE';
                } elseif ($catLower === 'tukar_unit') {
                    $exchangeInfo = $ueData->get($receiptId);
                    $exchangeType = 'TUKAR UNIT';
                } elseif ($catLower === 'refund') {
                    $exchangeInfo = $rfData->get($receiptId);
                    $exchangeType = 'REFUND';
                }
                ...
                if ($exchangeInfo && in_array($catLower, ['tukar_tambah', 'downgrade', 'tukar_unit'])) {
                    ...
                    if (!$isMulti) {
                        ...
                        // Add the single incoming item to the details array
                        $details[] = [
                            'name' => "IN: " . $inProd,
                            'qty' => 1,
                ...
                foreach ($trx->items as $item) {
                    ...
                    $details[] = [
                        ...
                        'qty' => 1,
                        'price' => $basePrice,
                        'original_price' => (float) ($item->pivot?->selling_price ?? $item->selling_price ?? 0),
                        'item_discount' => (float) ($item->pivot?->item_discount ?? 0),
                ...
                foreach ($trx->nonHpDetails as $item) {
                    ...
                    $details[] = [
                        ...
                        'qty' => $qty,
                        'price' => $price,
                        'original_price' => (float) ($item->selling_price ?? 0),
                ...
                if ($catLower === 'balancing' && empty($details)) {
                    ...
                    if ($originalTrx) {
                        if (!$trx->customer_wa)
                            $trx->customer_wa = $originalTrx->customer_wa ?? $originalTrx->customer_phone;
                        if (!$trx->customer_phone)
                            $trx->customer_phone = $originalTrx->customer_phone ?? $originalTrx->customer_wa;
                ...
                */
                ...
                if ($exchangeInfo) {
                    ...
                    if ($pOut == 0)
                        $pOut = (float) $trx->selling_price; // Fallback
                ...
                if ($oId && isset($branches[$oId])) {
                    ...
                    if ($u) {
                        if ($u->branch_id && isset($branches[$u->branch_id]))
                            $outletName = $branches[$u->branch_id]->name;
                        elseif ($u->online_shop_id && isset($onlineShops[$u->online_shop_id]))
                            $outletName = $onlineShops[$u->online_shop_id]->name;
                    }
                }
                ...
                return [
                    ...
                    'payment_proof_image' => $trx->payment_proof_image
                        ...
                        : null,
                    'payment_proof_images' => $trx->payment_proof_images ? collect($trx->payment_proof_images)->map(fn($img) => asset('storage/' . $img))->toArray() : [],
    ...
    public function exportSales(Request $request)
    {
        ...
        try {
            ...
            $locationName = 'ALL';
            if ($branchId)
                $locationName = \App\Models\Branch::find($branchId)?->name ?? 'CABANG';
            elseif ($onlineShopId)
                $locationName = \App\Models\OnlineShop::find($onlineShopId)?->name ?? 'ONLINE';
            ...
            if (count($xlsxData) > 1) {
                ...
                // Start summation from column index 21 (first payment/money column) 
                // up to the second-to-last column (Total Pengeluaran). Skip the final Status column.
                // Recalibrated from index 20 up to 21 (+1 for added column: keterangan)
                $startSumCol = 21;
            ...
            foreach ($groupedByOrder as $orderNo => $orderRows) {
                ...
                if (empty($outImeis) && empty($inImeis))
                    continue;
                ...
                if (in_array($cat, $combinedCategories)) {
                    ...
                    $imeiParts = [];
                    foreach ($outImeis as $oi)
                        $imeiParts[] = $oi . ' out';
                    foreach ($inImeis as $ii)
                        $imeiParts[] = $ii . ' in';
